 ...
 *
 * Test-only mock mode: DREAMBALL_EMBED_MOCK=hash returns a deterministic
 ...
 * In test mock mode (DREAMBALL_EMBED_MOCK=hash): returns deterministic hash-derived floats.
 *
 * TODO-EMBEDDING: bring-model-local-or-byo
...
export async function embedFor(
  ...
  // DREAMBALL_EMBED_MOCK='1' (dreamball-server convention, vite.config.ts) or
  // DREAMBALL_EMBED_MOCK='hash' (legacy embedding-client convention) both activate mock.
  // TODO-EMBEDDING: bring-model-local-or-byo
  const mockEnv = typeof process !== 'undefined' ? process.env.DREAMBALL_EMBED_MOCK : undefined;
 ...
 *             backwards compatibility. It uses DREAMBALL_EMBED_BASE env for the URL.
 *
...
export async function computeEmbedding(bytes: Uint8Array): Promise<number[]> {
  const base = (typeof process !== 'undefined' && process.env.DREAMBALL_EMBED_BASE)
    ? process.env.DREAMBALL_EMBED_BASE
    : 'http://localhost:9808';
 ...
 * ONLY used when DREAMBALL_EMBED_MOCK=hash (test mode).
