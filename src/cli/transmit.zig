//! `dreamball transmit <tool.jelly> --to=<target-fp-b58> --via-guild=<guild-fp-b58> --sender-key=<keyfile> --out=<transmission.jelly>`
//!
...
pub fn run(gpa: Allocator, argv: [][:0]const u8) !u8 {
    ...
    if (parsed.flag(4) or parsed.positional.items.len == 0) {
        try io.writeAllStdout(
            \\dreamball transmit <tool.jelly> --to <target-fp-b58> --via-guild <guild-fp-b58> --sender-key <keyfile> --out <receipt.jelly>
            \\
        );
        return 0;
    }
